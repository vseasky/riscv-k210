...
int main(void)
{
    ...
    lcd_init();
    lcd_clear(BLACK);
    lcd_draw_string(100, 60, "K210 Yolo Demo", BLUE);
    lcd_draw_string(90, 90, "Powered by vSeasky", BLUE);
    lcd_draw_string(100, 120, "www.liuwei.vin", BLUE);
    /* DVP init */
    printf("DVP init\n");
#if OV5640
    dvp_init(16);
    ...
    {
        printf("\nmodel init error\n");
        lcd_draw_string(94, 150, "model init error!", RED);
        while(1)
            ;
    }
    lcd_draw_string(90, 150, "model init success.", GREEN);
